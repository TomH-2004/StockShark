...
///
/// Binary format: each entry is 16 bytes (big-endian)
///   u64  key    — Polyglot Zobrist hash
///   u16  move   — encoded (to_file:3 to_rank:3 from_file:3 from_rank:3 promo:3)
///   u16  weight
///   u32  learn  (unused)
/// Entries are sorted ascending by key.
///
/// Official random table from polyglot-1.4 / random.c (public domain).
/// Verified: starting-position hash == 0x463B96181691FC9C.
///
/// Side-to-move convention (matches polyglot source):
///   XOR Random64[780] when WHITE is to move.

use std::fs;
use std::path::Path;

use crate::board::position::{Color, Piece, PieceType, Position, CR_WK, CR_WQ, CR_BK, CR_BQ};

// ── Official Polyglot Random64 table (781 values) ───────────────────────────
// Layout:
//   [0..767]   piece × sq  (piece 0=bPawn…11=wKing, sq 0=a1…63=h8)
...
static RANDOM64: [u64; 781] = [
    ...
    // side to move (XOR when WHITE to move — polyglot convention)
    0xF8D626AAAF278509,
];

/// Compute the Polyglot-compatible Zobrist hash for a position.
///
/// Piece index mapping (our bb_index XOR 1 = Polyglot index):
///   white pawn=1, black pawn=0, white knight=3, black knight=2, …, white king=11, black king=10
pub fn hash(pos: &Position) -> u64 {
    let mut h: u64 = 0;

    for sq in 0..64u8 {
        if let Some(piece) = pos.board[sq as usize] {
            // Our bb_index = piece_type*2 + color (0=white,1=black)
            // Polyglot index = bb_index ^ 1 (swaps white/black)
            let poly = piece.bb_index() ^ 1;
            h ^= RANDOM64[poly * 64 + sq as usize];
        }
    }

    // Castling rights
    if pos.castling & CR_WK != 0 { h ^= RANDOM64[768]; }
    if pos.castling & CR_WQ != 0 { h ^= RANDOM64[769]; }
    if pos.castling & CR_BK != 0 { h ^= RANDOM64[770]; }
    if pos.castling & CR_BQ != 0 { h ^= RANDOM64[771]; }

    // En passant: only include the file key when a pawn of the side to move can
    // actually make the capture (strict Polyglot spec).
    if pos.ep_sq != 255 {
        let ep_file = (pos.ep_sq % 8) as usize;
        let ep_rank = (pos.ep_sq / 8) as usize;
        // The capturing pawn sits one rank behind the ep square from the mover's perspective.
        let cap_rank = if pos.side == Color::White {
            ep_rank.wrapping_sub(1)
        } else {
            ep_rank + 1
        };
        if cap_rank < 8 {
            let can_capture = (ep_file > 0 && pos.board[cap_rank * 8 + ep_file - 1]
                    == Some(Piece::new(PieceType::Pawn, pos.side)))
                || (ep_file < 7 && pos.board[cap_rank * 8 + ep_file + 1]
                    == Some(Piece::new(PieceType::Pawn, pos.side)));
            if can_capture {
                h ^= RANDOM64[772 + ep_file];
            }
        }
    }

    // Side to move: XOR for WHITE (polyglot convention, opposite of what you'd expect).
...
impl PolyglotBook {
    /// Load a Polyglot .bin file.  Returns `None` if the file can't be read or
    ...
    /// Return all book moves for this position, sorted best-weight first.
    pub fn probe(&self, pos: &Position) -> Vec<Entry> {
        let key = hash(pos);
        let n   = self.data.len() / 16;
        ...
        while lo < hi {
            let mid = lo + (hi - lo) / 2;
            if self.read_key(mid) < key { lo = mid + 1; } else { hi = mid; }
        }

        let mut entries = Vec::new();
        let mut idx = lo;
        while idx < n && self.read_key(idx) == key {
            let mv16   = self.read_u16(idx * 16 + 8);
            let weight = self.read_u16(idx * 16 + 10);
            if weight > 0 {
                if let Some(uci) = decode_move(mv16) {
                    entries.push(Entry { uci, weight });
                }
            }
            idx += 1;
        }
    ...
    fn read_key(&self, idx: usize) -> u64 {
        let off = idx * 16;
...
fn decode_move(mv: u16) -> Option<String> {
    let to_file   = (mv       & 7) as u8;
    let to_rank   = ((mv >> 3) & 7) as u8;
    let from_file = ((mv >> 6) & 7) as u8;
    let from_rank = ((mv >> 9) & 7) as u8;
    let promo     = (mv >> 12) & 7;
    ...
    let mut from_sq = from_rank * 8 + from_file;
    let mut to_sq   = to_rank   * 8 + to_file;

    // Normalise castling encoded as king→rook to king→destination.
    (from_sq, to_sq) = match (from_sq, to_sq) {
        (4,  7) => (4,  6),  // e1h1 → e1g1  WK
        (4,  0) => (4,  2),  // e1a1 → e1c1  WQ
        (60, 63) => (60, 62), // e8h8 → e8g8  BK
        (60, 56) => (60, 58), // e8a8 → e8c8  BQ
        other   => other,
    };

    let from_str = sq_name(from_sq);
    let to_str   = sq_name(to_sq);
    Some(match promo {
        1 => format!("{}{}n", from_str, to_str),
        2 => format!("{}{}b", from_str, to_str),
        3 => format!("{}{}r", from_str, to_str),
        4 => format!("{}{}q", from_str, to_str),
        _ => format!("{}{}", from_str, to_str),
    })
}
...
mod tests {
    ...
    fn starting_position_hash() {
        ...
        assert_eq!(
            hash(&pos), 0x463B96181691FC9C,
            "Polyglot hash of starting position should be 0x463B96181691FC9C"
        );
    }
}
